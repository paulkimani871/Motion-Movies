...
function Home() {
  ...
  return (
    <div>
      <h1 className='text-white lg:text-3xl md:text-2xl text-xl font-bold text-center'> Find <span className='text-purple-400'> Movies </span> You'll Enjoy Without the Hassle</h1>
      <h3 className='text-purple-200 mt-7 ml-4 '>Top Movie Streaming Options</h3>
      <div className='flex mt-5 gap-5 ml-3 overflow-auto'>
      ...
      <div className='mr-3'>
        <h1 className='text-white text-2xl font-bold mt-10 ml-3'>All Movies</h1>
          ...
          <div className='flex gap-1 mt-5 ml-5 flex-wrap pb-20 w-full'>
         {movies?.map(movie=>( 
           <div key={movie.id} className=' md:w-[30%] lg:w-[19%] w-11/12'>
            <Link className='' to={`/details/movie/${movie.id}`}>
             <img className='rounded-[10px] w-11/12 mt-7 ml-7' src={`https://image.tmdb.org/t/p/w500${movie.poster_path}`} alt={movie.title} />
